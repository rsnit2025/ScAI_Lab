...
class AdaptiveDifferentialEvolutionWithDynamicPopulationV2:
    def __init__(self, max_evaluations=200000, dim=10, init_population_size=100, scaling_factor_range=(0.5, 2.0), crossover_rate_range=(0.1, 1.0)):
    ...
    def select_three_parents(self, population, current_idx):
        ...
        if len(idxs) < 3:
        # If not enough parents, use all available ones with replacement
          selected_idxs = np.random.choice(idxs, size=3, replace=True)
        else:  
    ...
    def update_parameters(self, scaling_factors, crossover_rates, fitness_values):
        scaling_factors *= np.exp(0.1 * (np.mean(fitness_values) - fitness_values) / (np.std(fitness_values) + 1e-6))
        crossover_rates *= np.exp(0.1 * (np.mean(fitness_values) - fitness_values) / (np.std(fitness_values) + 1e-6))
    ...
    def optimize(self, func):
        ...
        population = np.random.uniform(self.bounds[0], self.bounds[1], size=(population_size, dimension))
        fitness_values = np.array([func.evaluate(ind) for ind in population])
        self.eval_count += population_size

        scaling_factors = np.random.uniform(self.scaling_factor_range[0], self.scaling_factor_range[1], size=population_size)
        crossover_rates = np.random.uniform(self.crossover_rate_range[0], self.crossover_rate_range[1], size=population_size)
        ...
        while self.eval_count < self.max_evaluations:
            for i in range(population_size):
                ...
                p_best_idx = np.random.choice(
                    np.delete(np.arange(population_size), i))
                p_best = population[p_best_idx]

                mutant = np.clip(a + scaling_factors[i] * (b - c) + scaling_factors[i] * (p_best - population[i]),
                ...
                trial_fitness = func.evaluate(trial_individual)
                self.eval_count += 1
            ...
            scaling_factors, crossover_rates = self.update_parameters(scaling_factors, crossover_rates, fitness_values)
            ...
            if np.random.rand() < 0.1:
                if population_size > 2:
                    ...
                    population_size += 1
                    new_individual = np.random.uniform(self.bounds[0], self.bounds[1], size=dimension)
                    population = np.vstack([population, new_individual])
                    fitness_values = np.append(fitness_values, func.evaluate(new_individual))
                    scaling_factors = np.append(scaling_factors, np.random.uniform(*self.scaling_factor_range))
                    crossover_rates = np.append(crossover_rates, np.random.uniform(*self.crossover_rate_range))
                    self.eval_count += 1
...
def comprehensive_statistical_test():
    ...
    print("=" * 90)
    print("ADAPTIVE DIFFERENTIAL EVOLUTION WITH DYNAMIC POPULATION V2 - CEC 2022 BENCHMARK")
    ...
    for dim_idx, dimension in enumerate(dimensions_to_test):
        ...
        for func_idx, FuncClass in enumerate(all_functions):
            ...
            for run in range(num_runs_per_config):
                ...
                ade = AdaptiveDifferentialEvolutionWithDynamicPopulationV2(
                ...
                final_F_values.append(np.mean(ade.scaling_factor_range))
                final_CR_values.append(np.mean(ade.crossover_rate_range))
                ...
                detailed_run_data.append({
                    ...
                    'Final_F': np.mean(ade.scaling_factor_range),
                    'Final_CR': np.mean(ade.crossover_rate_range),
    ...
    summary_filename = f"adaptive_de_dynamic_pop_v2_statistical_summary_{timestamp}.csv"
    detailed_filename = f"adaptive_de_dynamic_pop_v2_detailed_runs_{timestamp}.csv"
...
def generate_visualizations(summary_df, timestamp):
    ...
    plt.tight_layout()
    plt.savefig(f'adaptive_de_dynamic_pop_v2_analysis_plots_{timestamp}.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"Visualizations saved to: adaptive_de_dynamic_pop_v2_analysis_plots_{timestamp}.png")
...
def quick_test_single_function():
    ...
    print(f"Algorithm: Adaptive Differential Evolution with Dynamic Population V2")
    ...
    print(f"Scaling Factor Range: {ade.scaling_factor_range}")
    print(f"Crossover Rate Range: {ade.crossover_rate_range}")
...
def parameter_sensitivity_analysis():
    ...
    param_combinations = [
        {'init_population_size': 30, 'scaling_factor_range': (0.3, 1.5), 'crossover_rate_range': (0.2, 0.8)},
        {'init_population_size': 50, 'scaling_factor_range': (0.5, 2.0), 'crossover_rate_range': (0.1, 1.0)},
        {'init_population_size': 70, 'scaling_factor_range': (0.7, 2.5), 'crossover_rate_range': (0.05, 0.9)},
        {'init_population_size': 40, 'scaling_factor_range': (0.4, 1.8), 'crossover_rate_range': (0.15, 0.95)},
    ]
    
    results = []
    
    for i, params in enumerate(param_combinations):
        print(f"\nTest {i+1}: pop_size={params['init_population_size']}, F_range={params['scaling_factor_range']}, CR_range={params['crossover_rate_range']}")
        ...
        for run in range(5):  # 5 runs per configuration
            ade = AdaptiveDifferentialEvolutionWithDynamicPopulationV2(
                max_evaluations=max_evaluations,
                dim=10,
                init_population_size=params['init_population_size'],
                scaling_factor_range=params['scaling_factor_range'],
                crossover_rate_range=params['crossover_rate_range']
            )
...
if __name__ == "__main__":
    print("Adaptive Differential Evolution with Dynamic Population V2 - CEC 2022 Testing")
    print("=" * 70)
    ...
    print("This will test Adaptive Differential Evolution with Dynamic Population V2")
    ...
    if response.lower() == 'y':
        ...
        print(f"\n{'=' * 90}")
        print("FINAL SUMMARY - ADAPTIVE DIFFERENTIAL EVOLUTION WITH DYNAMIC POPULATION V2")
        print(f"{'=' * 90}")
        ...
        print(f"Initial Population Size: 50")
        print(f"Scaling Factor Range: (0.5, 2.0)")
        print(f"Crossover Rate Range: (0.1, 1.0)")
