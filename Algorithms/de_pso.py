...
class AdaptiveDifferentialEvolutionPSO:
    ...
    def crossover(self, target, mutant, CR):
        ...
        trial = np.array([mutant[j] if np.random.rand() < CR or j == j_rand else target[j] for j in range(self.dim)])
        return trial

    def optimize(self, func):
        self.f_opt = np.inf
        self.x_opt = None
        self.min_pop_size = min(self.min_pop_size, self.initial_pop_size)

        population, velocities = self.initialize_population()
        personal_best_positions = np.copy(population)
        personal_best_scores = np.array([func.evaluate(ind) for ind in population])
        global_best_position = personal_best_positions[np.argmin(personal_best_scores)]
        global_best_score = np.min(personal_best_scores)
        evaluations = self.initial_pop_size
        
        while evaluations < self.max_evaluations:
            current_pop_size = max(self.min_pop_size, int(self.initial_pop_size * ((self.max_evaluations - evaluations) / self.max_evaluations)))
            new_population = np.zeros((current_pop_size, self.dim))
            ...
            for i in range(current_pop_size):
                ...
                fitness[i] = func.evaluate(new_population[i])
                evaluations += 1
...
def comprehensive_statistical_test():
    ...
    print("=" * 90)
    print("ADAPTIVE DIFFERENTIAL EVOLUTION PSO - CEC 2022 BENCHMARK")
    ...
    for dim_idx, dimension in enumerate(dimensions_to_test):
        ...
        for func_idx, FuncClass in enumerate(all_functions):
            ...
            for run in range(num_runs_per_config):
                ...
                ade = AdaptiveDifferentialEvolutionPSO(
                ...
                final_F_values.append(ade.initial_F)
                final_CR_values.append(ade.initial_CR)
                ...
                detailed_run_data.append({
                    ...
                    'Final_F': ade.initial_F,
                    'Final_CR': ade.initial_CR,
    ...
    summary_filename = f"adaptive_de_pso_statistical_summary_{timestamp}.csv"
    detailed_filename = f"adaptive_de_pso_detailed_runs_{timestamp}.csv"
...
def generate_visualizations(summary_df, timestamp):
    ...
    plt.tight_layout()
    plt.savefig(f'adaptive_de_pso_analysis_plots_{timestamp}.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"Visualizations saved to: adaptive_de_pso_analysis_plots_{timestamp}.png")
...
def quick_test_single_function():
    ...
    print(f"Algorithm: Adaptive Differential Evolution PSO")
    ...
    print(f"Initial Mutation Factor: {ade.initial_F:.3f}")
    print(f"Initial Crossover Rate: {ade.initial_CR:.3f}")
...
def parameter_sensitivity_analysis():
    ...
    param_combinations = [
        {'initial_pop_size': 40, 'min_pop_size': 15},
        {'initial_pop_size': 60, 'min_pop_size': 20},
        {'initial_pop_size': 80, 'min_pop_size': 25},
        {'initial_pop_size': 100, 'min_pop_size': 30},
    ]
    
    results = []
    
    for i, params in enumerate(param_combinations):
        print(f"\nTest {i+1}: initial_pop_size={params['initial_pop_size']}, min_pop_size={params['min_pop_size']}")
        
        # Note: Need to modify algorithm to accept these parameters
        # For now, using default algorithm
        run_errors = []
...
if __name__ == "__main__":
    print("Adaptive Differential Evolution PSO - CEC 2022 Testing")
    print("=" * 70)
    ...
    print("This will test Adaptive Differential Evolution PSO")
    ...
    if response.lower() == 'y':
        ...
        print(f"\n{'=' * 90}")
        print("FINAL SUMMARY - ADAPTIVE DIFFERENTIAL EVOLUTION PSO")
        print(f"{'=' * 90}")
        ...
        print(f"Initial Population Size: 60, Min Population Size: 20")
        print(f"Initial F: 0.5, Initial CR: 0.9")
        print(f"c1: 1.5, c2: 1.5, w: 0.5")
