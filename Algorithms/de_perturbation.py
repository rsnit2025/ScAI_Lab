...
class AdaptiveDifferentialEvolutionWithAdaptivePerturbation:
    def __init__(self, max_evaluations, dim=10, population_size=100, crossover_rate=0.7, mutation_factor=0.8):
    ...
    def gradient_estimate(self, x, func, h=1e-5):
        ...
        for i in range(len(x)):
            ...
            x1[i] += h
            x2[i] -= h
            grad[i] = (func.evaluate(x1) - func.evaluate(x2)) / (2 * h)
            self.eval_count += 2
        return grad

    def maintain_diversity(self, population, fitness, func):
        for i in range(len(population)):
            for j in range(i + 1, len(population)):
                if np.linalg.norm(population[i] - population[j]) < 1e-3:
                    if fitness[i] > fitness[j]:
                        population[i] = self.random_vector()
                        fitness[i] = func.evaluate(population[i])
                        self.eval_count += 1
                    else:
                        population[j] = self.random_vector()
                        fitness[j] = func.evaluate(population[j])
                        self.eval_count += 1
    ...
    def optimize(self, func):
        ...
        population = [self.random_vector() for _ in range(self.population_size)]
        fitness = [func.evaluate(ind) for ind in population]
        ...
        while self.eval_count < self.max_evaluations:
            ...
            for j in range(self.population_size):
                ...
                target = population[j]
                a, b, c = self.select_parents(population, fitness)
                ...
                grad = self.gradient_estimate(trial, func)
            ...
            # Maintain diversity
            self.maintain_diversity(population, fitness, func)
...
def comprehensive_statistical_test():
    ...
    print("=" * 90)
    print("ADAPTIVE DIFFERENTIAL EVOLUTION WITH ADAPTIVE PERTURBATION - CEC 2022 BENCHMARK")
    ...
    for dim_idx, dimension in enumerate(dimensions_to_test):
        ...
        for func_idx, FuncClass in enumerate(all_functions):
            ...
            for run in range(num_runs_per_config):
                ...
                ade = AdaptiveDifferentialEvolutionWithAdaptivePerturbation(
                ...
                final_F_values.append(ade.mutation_factor)
                ...
                detailed_run_data.append({
                    ...
                    'Final_F': ade.mutation_factor,
                    'Final_CR': ade.crossover_rate,
                    'Evaluations_Used': ade.eval_count,
                    'Final_Learning_Rate': ade.base_lr
                })
    ...
    summary_filename = f"adaptive_de_perturbation_statistical_summary_{timestamp}.csv"
    detailed_filename = f"adaptive_de_perturbation_detailed_runs_{timestamp}.csv"
...
def generate_visualizations(summary_df, timestamp):
    ...
    plt.tight_layout()
    plt.savefig(f'adaptive_de_perturbation_analysis_plots_{timestamp}.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"Visualizations saved to: adaptive_de_perturbation_analysis_plots_{timestamp}.png")
...
def quick_test_single_function():
    ...
    print(f"Algorithm: Adaptive Differential Evolution with Adaptive Perturbation")
    ...
    print(f"Final Mutation Factor: {ade.mutation_factor:.3f}")
    print(f"Final Crossover Rate: {ade.crossover_rate:.3f}")
    print(f"Final Learning Rate: {ade.base_lr:.6f}")
...
def parameter_sensitivity_analysis():
    ...
    param_combinations = [
        {'mutation_factor': 0.5, 'crossover_rate': 0.5, 'base_lr': 0.05},
        {'mutation_factor': 0.8, 'crossover_rate': 0.7, 'base_lr': 0.1},
        {'mutation_factor': 1.0, 'crossover_rate': 0.9, 'base_lr': 0.2},
        {'mutation_factor': 0.6, 'crossover_rate': 0.6, 'base_lr': 0.15},
    ]
    
    results = []
    
    for i, params in enumerate(param_combinations):
        print(f"\nTest {i+1}: mutation_factor={params['mutation_factor']}, crossover_rate={params['crossover_rate']}, base_lr={params['base_lr']}")
...
if __name__ == "__main__":
    print("Adaptive Differential Evolution with Adaptive Perturbation - CEC 2022 Testing")
    print("=" * 70)
    ...
    print("This will test Adaptive Differential Evolution with Adaptive Perturbation")
    ...
    if response.lower() == 'y':
        ...
        print(f"\n{'=' * 90}")
        print("FINAL SUMMARY - ADAPTIVE DIFFERENTIAL EVOLUTION WITH ADAPTIVE PERTURBATION")
        print(f"{'=' * 90}")
        ...
        print(f"Population Size: 20, Mutation Factor: 0.8, Crossover Rate: 0.7")
        print(f"Base Learning Rate: 0.1, Epsilon: 1e-8")
        print(f"Average Final Mutation Factor: {detailed_df['Final_F'].mean():.3f}")
