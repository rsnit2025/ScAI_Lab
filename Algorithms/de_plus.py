...
class AdaptiveDifferentialEvolutionPlus:
    def __init__(self, max_evaluations=200000, dim=10, population_size=100):
    ...
    def optimize(self, func):
        ...
        fitness = np.array([func.evaluate(individual) for individual in population])
...
def comprehensive_statistical_test():
    ...
    print("=" * 90)
    print("ADAPTIVE DIFFERENTIAL EVOLUTION PLUS - CEC 2022 BENCHMARK")
    ...
    for dim_idx, dimension in enumerate(dimensions_to_test):
        ...
        for func_idx, FuncClass in enumerate(all_functions):
            ...
            for run in range(num_runs_per_config):
                ...
                ade = AdaptiveDifferentialEvolutionPlus(
                ...
                final_CR_values.append(ade.crossover_probability)
                ...
                detailed_run_data.append({
                    ...
                    'Final_CR': ade.crossover_probability,
    ...
    summary_filename = f"adaptive_de_plus_statistical_summary_{timestamp}.csv"
    detailed_filename = f"adaptive_de_plus_detailed_runs_{timestamp}.csv"
...
def generate_visualizations(summary_df, timestamp):
    ...
    plt.tight_layout()
    plt.savefig(f'adaptive_de_plus_analysis_plots_{timestamp}.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"Visualizations saved to: adaptive_de_plus_analysis_plots_{timestamp}.png")
...
def quick_test_single_function():
    ...
    print(f"Algorithm: Adaptive Differential Evolution Plus")
    ...
    print(f"Final Crossover Probability: {ade.crossover_probability:.3f}")
...
def parameter_sensitivity_analysis():
    ...
    param_combinations = [
        {'population_size': 10, 'adaptive_rate': 0.05},
        {'population_size': 20, 'adaptive_rate': 0.1},
        {'population_size': 30, 'adaptive_rate': 0.15},
        {'population_size': 40, 'adaptive_rate': 0.2},
    ]
    
    results = []
    
    for i, params in enumerate(param_combinations):
        print(f"\nTest {i+1}: pop_size={params['population_size']}, adaptive_rate={params['adaptive_rate']}")
        
        # Note: Need to modify algorithm to accept adaptive_rate parameter
...
if __name__ == "__main__":
    print("Adaptive Differential Evolution Plus - CEC 2022 Testing")
    print("=" * 70)
    ...
    print("This will test Adaptive Differential Evolution Plus")
    ...
    if response.lower() == 'y':
        ...
        print(f"\n{'=' * 90}")
        print("FINAL SUMMARY - ADAPTIVE DIFFERENTIAL EVOLUTION PLUS")
        print(f"{'=' * 90}")
        ...
        print(f"Initial Mutation Factor: 0.8, Initial Crossover Probability: 0.9")
        print(f"Adaptive Rate: 0.1, Population Size: 20")
        print(f"Average Final Mutation Factor: {detailed_df['Final_F'].mean():.3f}")
        print(f"Average Final Crossover Probability: {detailed_df['Final_CR'].mean():.3f}")
