...
class AdaptiveDifferentialEvolutionHarmonySearch:
    def __init__(self, max_evaluations=200000, dim=10, harmony_memory_size=20, hmcr=0.9, par=0.4, bw=0.5, bw_decay=0.95, f_weight=0.8, cr=0.9):
    ...
    def initialize_harmony_memory(self, func):
        ...
        self.harmony_memory_fitness = np.array([func.evaluate(x) for x in self.harmony_memory])
        self.eval_count += self.harmony_memory_size

    def differential_evolution(self, func):
        idxs = np.random.choice(self.harmony_memory_size, 3, replace=False)
        v = self.harmony_memory[idxs[0]] + self.f_weight * (self.harmony_memory[idxs[1]] - self.harmony_memory[idxs[2]])
        ...
        while self.eval_count < self.max_evaluations:
            ...
            for j in range(self.dim):
                if np.random.rand() < self.hmcr:
                    ...
                    new_harmony[j] = np.random.uniform(self.bounds[0], self.bounds[1])

                if np.random.rand() < self.par:
                    new_harmony[j] += self.bw * np.random.randn()

                new_harmony[j] = np.clip(new_harmony[j], self.bounds[0], self.bounds[1])

            new_fitness = func.evaluate(new_harmony)
            self.eval_count += 1
            ...
            de_fitness = func.evaluate(de_harmony)
            self.eval_count += 1
...
def comprehensive_statistical_test():
    ...
    print("=" * 90)
    print("ADAPTIVE DIFFERENTIAL EVOLUTION HARMONY SEARCH - CEC 2022 BENCHMARK")
    ...
    for dim_idx, dimension in enumerate(dimensions_to_test):
        ...
        for func_idx, FuncClass in enumerate(all_functions):
            ...
            for run in range(num_runs_per_config):
                ...
                ade = AdaptiveDifferentialEvolutionHarmonySearch(
                ...
                final_F_values.append(ade.f_weight)
                final_CR_values.append(ade.cr)
    ...
    summary_filename = f"adaptive_de_hs_statistical_summary_{timestamp}.csv"
    detailed_filename = f"adaptive_de_hs_detailed_runs_{timestamp}.csv"
...
def generate_visualizations(summary_df, timestamp):
    ...
    plt.tight_layout()
    plt.savefig(f'adaptive_de_hs_analysis_plots_{timestamp}.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"Visualizations saved to: adaptive_de_hs_analysis_plots_{timestamp}.png")
...
def quick_test_single_function():
    ...
    print(f"Algorithm: Adaptive Differential Evolution Harmony Search")
    ...
    print(f"Final F Weight: {ade.f_weight:.3f}")
    print(f"Final CR: {ade.cr:.3f}")
    print(f"Final Bandwidth: {ade.bw:.6f}")
...
def parameter_sensitivity_analysis():
    ...
    param_combinations = [
        {'hmcr': 0.7, 'par': 0.3, 'f_weight': 0.5},
        {'hmcr': 0.9, 'par': 0.4, 'f_weight': 0.8},
        {'hmcr': 0.95, 'par': 0.5, 'f_weight': 1.0},
        {'hmcr': 0.8, 'par': 0.35, 'f_weight': 0.6},
    ]
    
    results = []
    
    for i, params in enumerate(param_combinations):
        print(f"\nTest {i+1}: hmcr={params['hmcr']}, par={params['par']}, f_weight={params['f_weight']}")
...
if __name__ == "__main__":
    print("Adaptive Differential Evolution Harmony Search - CEC 2022 Testing")
    print("=" * 70)
    ...
    print("This will test Adaptive Differential Evolution Harmony Search")
    ...
    if response.lower() == 'y':
        ...
        print(f"\n{'=' * 90}")
        print("FINAL SUMMARY - ADAPTIVE DIFFERENTIAL EVOLUTION HARMONY SEARCH")
        print(f"{'=' * 90}")
        ...
        print(f"Harmony Memory Size: 20, HMCR: 0.9, PAR: 0.4")
        print(f"Initial Bandwidth: 0.5, Bandwidth Decay: 0.95")
        print(f"F Weight: 0.8, CR: 0.9")
