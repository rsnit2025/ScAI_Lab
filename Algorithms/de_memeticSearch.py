...
class AdaptiveDifferentialEvolutionWithMemeticSearch:
    def __init__(self, max_evaluations=10000, dim=10):
    ...
    def compute_gradient(self, func, x):
        ...
        for i in range(self.dim):
            ...
            x_upper[i] += epsilon
            x_lower[i] -= epsilon
            f_upper = func.evaluate(x_upper)
            f_lower = func.evaluate(x_lower)
    ...
    def optimize(self, func):
        ...
        while self.eval_count < self.max_evaluations:
            ...
            if self.eval_count < self.max_evaluations:
                local_search_budget = int(self.max_evaluations * 0.1)  # allocate 10% of the budget for local search
                for _ in range(local_search_budget):
                    perturbation = np.random.normal(0, 0.1, self.dim)
                    local_trial = np.clip(self.x_opt + perturbation, bounds[0], bounds[1])
                    f_local_trial = func.evaluate(local_trial)
                    self.eval_count += 1
            ...
            if self.eval_count < self.max_evaluations:
                local_search_budget = int(self.max_evaluations * 0.1)  # allocate another 10% of the budget for local search
                for _ in range(local_search_budget):
...
def comprehensive_statistical_test():
    ...
    print("=" * 90)
    print("ADAPTIVE DIFFERENTIAL EVOLUTION WITH MEMETIC SEARCH - CEC 2022 BENCHMARK")
    ...
    for dim_idx, dimension in enumerate(dimensions_to_test):
        ...
        for func_idx, FuncClass in enumerate(all_functions):
            ...
            for run in range(num_runs_per_config):
                ...
                ade = AdaptiveDifferentialEvolutionWithMemeticSearch(
                ...
                final_F_values.append(np.mean(ade.F_values) if hasattr(ade, 'F_values') else 0.8)
                final_CR_values.append(np.mean(ade.CR_values) if hasattr(ade, 'CR_values') else 0.9)
                ...
                detailed_run_data.append({
                    ...
                    'Final_F': np.mean(ade.F_values) if hasattr(ade, 'F_values') else 0.8,
                    'Final_CR': np.mean(ade.CR_values) if hasattr(ade, 'CR_values') else 0.9,
    ...
    summary_filename = f"adaptive_de_memetic_search_statistical_summary_{timestamp}.csv"
    detailed_filename = f"adaptive_de_memetic_search_detailed_runs_{timestamp}.csv"
...
def generate_visualizations(summary_df, timestamp):
    ...
    plt.tight_layout()
    plt.savefig(f'adaptive_de_memetic_search_analysis_plots_{timestamp}.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"Visualizations saved to: adaptive_de_memetic_search_analysis_plots_{timestamp}.png")
...
def quick_test_single_function():
    ...
    print(f"Algorithm: Adaptive Differential Evolution with Memetic Search")
    ...
    print(f"Population Size: 20, Initial F: 0.8, Initial CR: 0.9")
...
def parameter_sensitivity_analysis():
    ...
    param_combinations = [
        {'initial_F': 0.5, 'initial_CR': 0.7},
        {'initial_F': 0.8, 'initial_CR': 0.9},
        {'initial_F': 1.0, 'initial_CR': 0.5},
        {'initial_F': 0.6, 'initial_CR': 0.8},
    ]
    
    results = []
    
    for i, params in enumerate(param_combinations):
        print(f"\nTest {i+1}: F={params['initial_F']}, CR={params['initial_CR']}")
        
        # Note: The algorithm uses self-adaptive parameters, so initial values may be overridden
        run_errors = []
...
if __name__ == "__main__":
    print("Adaptive Differential Evolution with Memetic Search - CEC 2022 Testing")
    print("=" * 70)
    ...
    print("This will test Adaptive Differential Evolution with Memetic Search")
    ...
    if response.lower() == 'y':
        ...
        print(f"\n{'=' * 90}")
        print("FINAL SUMMARY - ADAPTIVE DIFFERENTIAL EVOLUTION WITH MEMETIC SEARCH")
        print(f"{'=' * 90}")
        ...
        print(f"Population Size: 20, Initial F: 0.8, Initial CR: 0.9")
        print(f"Local Search Budget: 20% of total evaluations (10% random + 10% gradient-based)")
